...
// MGB insert skip code blocks and tags Here XXXXXXXXXXXXXXXXXXXXXXX

    let mes = message.mes;

    if (extension_settings.tts.skip_codeblocks) {
        mes = mes
            .replace(/^\s{4}.*$/gm, '')           // indented code
            .replace(/```[\s\S]*?```/g, '')       // fenced ```
            .replace(/~~~[\s\S]*?~~~/g, '');      // fenced ~~~
    }

    if (extension_settings.tts.skip_tags) {
        mes = mes.replace(/<[^>]+>[\s\S]*?<\/[^>]+>/g, ''); // remove <tag>...</tag>
    }

// MGB insert skip code blocks and tags Here XXXXXXXXXXXXXXXXXXXXXXX


// MGB comment-out    const lines = message.mes.split('\n');
	
	const lines = mes.split('\n');
...
// MGB Previous Block XXXXXXXXXXXXXXXXXXXXXXX	
...
// MGB FIX Block XXXXXXXXXXXXXXXXXXXXXXX
// prevent handler stacking — use property assignments (replaces prior handlers)
    audioElement.onended = completeCurrentAudioJob; 
...
// MGB Previous Block XXXXXXXXXXXXXXXXXXXXXXXXXXXX
//            if (!voiceMapEntry || voiceMapEntry === DISABLED_VOICE_MARKER) {
//                throw `${char} not in voicemap. Configure character in extension settings voice map`;
//            }
// MGB Previous Block XXXXXXXXXXXXXXXXXXXXXXXXXXXX		

// MGB New Block EDIT XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX		

        if (!voiceMapEntry || voiceMapEntry === DISABLED_VOICE_MARKER) {
			console.log(`MGB Here voiceMap Line 541: ${voiceMapEntry}`);
            // throw `${char} not in voicemap. Configure character in extension settings voice map`;
			console.warn(`${char} not in voicemap. Configure character in extension settings voice map`);
            completeTtsJob();
            return;
        }
...
// MGB New Block EDIT XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX	
	...
	// skips text enclosed in code blocks
    if (extension_settings.tts.skip_codeblocks) {
...
//XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX MGB EDIT NEW BLOCK XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
	...
	if (!extension_settings.tts.pass_asterisks) {
		text = extension_settings.tts.narrate_dialogues_only
			? text.replace(/\*[^*]*?(\*|$)/g, '').replace(/\[[^\]]*?\]/g, '').replace(/_/g, '').replace(/#/g, '').replace(/°/g, '').replace(/\|/g, '').replace(/	/g, '').replace(/ {7}/g, '').replace(/ {6}/g, '').replace(/ {5}/g, '').replace(/ {4}/g, '').replace(/ {3}/g, '').replace(/ {2}/g, ' ').replace(/-{3}/g, '').trim() // remove asterisks content, bracket content, underscores, hashtags, degrees, and vertical bars* .replace(/—/g, '')
			: text.replaceAll('*', '').replace(/\[[^\]]*?\]/g, '').replace(/_/g, '').replace(/#/g, '').replace(/°/g, '').replace(/\|/g, '').replace(/	/g, '').replace(/ {7}/g, '').replace(/ {6}/g, '').replace(/ {5}/g, '').replace(/ {4}/g, '').replace(/ {3}/g, '').replace(/ {2}/g, ' ').replace(/-{3}/g, '').trim(); // remove just the asterisks, bracket content, underscores, hashtags, degrees, and vertical bars
	}
	

	//text = text.replace(/\^([^^]+)\^/g, '[$1]'); // convert ^emote^ markers into [emote] for ElevenLabs

    if (extension_settings.tts.ignore_parenthesis) {
        
		text = text.replace(/\([^)]*?\)/g, '').trim(); // remove content enclosed in parenthesis ( )
    }
...
function loadSettings() {
	...
	$('#tts_ignore_parenthesis').prop('checked', extension_settings.tts.ignore_parenthesis); // MGB Edit Add Line tts_ignore_parenthesis XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX	
...
// MGB Edit New Block Add XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX

function onIgnoreParenthesisClick() {
    extension_settings.tts.ignore_parenthesis = !!$('#tts_ignore_parenthesis').prop('checked');
    saveSettingsDebounced();
}
...
jQuery(async function () {
    async function addExtensionControls() {
		...
		$('#tts_ignore_parenthesis').on('click', onIgnoreParenthesisClick); // MGB Edit New Line Add in above block tts_ignore_parenthesis XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX		
